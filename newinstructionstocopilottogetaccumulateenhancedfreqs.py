    ...
    for synset in list(wn.all_synsets()):
        for lemma in synset.lemmas():
            try:
                ...
                word_frequency = word_freq.get(lemma.name(), 0)  # Get the word frequency   #i need to sum with aother frequencies in same row
                
                row = {
                    'Synset ID': synset.name(),
                    'Word': lemma.name(), # #i need to sum with other sensitivity of same row total     in same row accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens
                    'Part of Speech': synset.pos(), # #i need to sum with other sensitivity of same row total     in same row accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens
                    'Definition': synset.definition(),  # #i need to sum with other sensitivity of same row total     in same row accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens
                    'Examples': ', '.join(synset.examples()),  # #i need to sum with other sensitivity of same row total     # accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens
                    'Synonyms': ', '.join([l.name() for l in synset.lemmas()]),  # #i need to sum with other sensitivity of same row total     # accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens
                    'Antonyms': ', '.join([ant.name() for ant in lemma.antonyms()]),  # #i need to sum with other sensitivity of same row total     # accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens
                    'Hypernyms': ', '.join([h.name() for h in synset.hypernyms()]),  # #i need to sum with other sensitivity of same row total     # accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens
###                    'Hyponyms': ', '.join([h.name() for h in synset.hyponyms()]),  # #i need to sum with other sensitivity of same row total     # accumulate
					
                    'Hyponyms': ', '.join([h.name() for h in synset.hyponyms()]),   # #i need to sum with other sensitivity of same row total     in same row accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens# accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens
                    'Meronyms': ', '.join([m.name() for m in synset.part_meronyms()]),   # #i need to sum with other sensitivity of same row total     in same row accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens # #i need to sum with other sensitivity of same row total     in same row accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens# accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens
                    'Holonyms': ', '.join([h.name() for h in synset.part_holonyms()]),  # #i need to sum with other sensitivity of same row total     in same row accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens # accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens
                    'Troponyms': troponyms,# #i need to sum with other sensitivity of same row total     in same row accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens
                    'Derivationally Related Forms': ', '.join([d.name() for d in lemma.derivationally_related_forms()]),# #i need to sum with other sensitivity of same row total     in same row accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens
                    'Lexical Relations': ', '.join([l.name() for l in synset.lemmas()]),# #i need to sum with other sensitivity of same row total     in same row accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens
                    'Word Frequency': word_frequency,  # Word frequency from the calculated word frequencies
                    'Senses': len(synset.lemmas()),# #i need to sum with other sensitivity of same row total     in same row accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens
                    'Sentiment': calculate_sentiment(synset.definition()),# #i need to sum with other sensitivity of same row total     in same row accumulate all the tokens and add the frequency of each of the tokens and also add the sensitivity for each of the tokens
...
def main():
    ...
    try:
        ...
        # Export to Excel
        wordnet_df.to_excel('copilotstextdumpsfirstwordnet_data_with_sensitivity_and_frequency.xlsx', index=False, engine='openpyxl')
