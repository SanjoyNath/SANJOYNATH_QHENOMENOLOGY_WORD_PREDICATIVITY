    ...
    for synset in wn.all_synsets():
        for lemma in synset.lemmas():
            try:
                ...
                tokens = word_tokenize(row_text)###split the tokens with special characters ike dash , underscores or spaces or whatever non alphabets to calculate frequencies and to calculate sensitivity , to calculate frequencies and also to calculate frequency take the word nside other word as substring then add that count also
...
def main():
    try:
        ...
        wordnet_data.to_excel("withsubstringsnonspecialcharsrevisedweightsdescendingonfreqsprodssensts_enhanced_wordnet_data_sorted.xlsx", index=False, engine='openpyxl')
