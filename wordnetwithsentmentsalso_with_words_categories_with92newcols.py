    ...
    for word in words:
        ...
        for meaning_counter, synset in enumerate(synsets, start=1):
            ...
            for category in categories:
                row[f"{category} Unique Frequency"] = category_frequencies_current_row.get(category, 0)
                row[f"{category} Total Frequency"] = category_frequencies_all_meanings.get(category, 0)
...
generate_combined_report(
    output_file="wordnet_combined_report.txt",
    excel_file="trying_233362rows_with_92_newcols_wordnet_combined_report.xlsx"
)
