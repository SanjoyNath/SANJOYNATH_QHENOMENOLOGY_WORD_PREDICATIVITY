...
# Define ancientness sensitivity values (weights for terms)
ancientness_sensitivity_values = {
    'Adj all': 600, 'Adj pert': 550, 'Adj ppl': 500, 'Adv all': 450,
    'Noun act': 400, 'Noun animal': 660, 'Noun artifact': 680, 'Noun attribute': 250,
    ...
    'Verb body': 606, 'Verb change': 550, 'Verb cognition': 500, 'Verb communication': 450,
    'Verb competition': 400, 'Verb consumption': 350, 'Verb contact': 300, 'Verb creation': 250,
    'Verb emotion': 221, 'Verb motion': 150, 'Verb perception': 100, 'Verb possession': 90,
...
# Collect WordNet data with predicativity check
def collect_wordnet_data(word_freq):
    data = []
    conflicts = []  # To store predicativity errors
    for synset in wn.all_synsets():
        for lemma in synset.lemmas():
            try:
                row_text = f"{synset.definition()} {' '.join(synset.examples())} {' '.join([l.name() for l in synset.lemmas()])}"
                tokens = word_tokenize(row_text)  # Split the tokens with special characters
                ...
                # Check for predicativity errors (circular references)
                for token in tokens:
                    token_sens = word_freq.get(token.lower(), 0) * ancientness_sensitivity_values.get(token.lower(), 0)
                    if token_sens < frequency_sensitivity_sum:
                        conflicts.append(f"Conflict: {token} used in lower sensitivity row in {synset.name()}")
                ...
                row = {
                    ...
                    'Frequency * SensitivitySum': frequency_sensitivity_sum,
                    'Predicativity Conflict': "; ".join(conflicts)  # Add conflict column
                }
                data.append(row)
            except Exception as e:
                logging.error(f"Error processing synset {synset.name()} and lemma {lemma.name()}: {e}")
...
def main():
    try:
        ...
        wordnet_data.to_excel("epredicativitychecking___nhanced_wordnet_data_with_predicativity_check.xlsx", index=False, engine='openpyxl')
        print("Data exported successfully with predicativity check.")
    except Exception as e:
