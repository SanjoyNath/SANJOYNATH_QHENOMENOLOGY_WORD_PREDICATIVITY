        ...
        for lemma in synset.lemmas():
            try:
                ...
                row = {
                    ...
                    'SensitivitySum': sum(sensitivity_sum.values()),###??? why dont you generate the columns for Frequency * SensitivitySum ??? in excel it truncates so i need descending sorted on these data through python
                }
                data.append(row)
            except Exception as e:
                logging.error(f"Error processing synset {synset.name()} and lemma {lemma.name()}: {e}")
...
def main():
    try:
        ...
        wordnet_data.to_excel("withrowtotalsfreqsrowtotalssensitivitiesenhanced_wordnet_data.xlsx", index=False, engine='openpyxl')
