...
def count_pos_and_sensitivity(tokens):
    ...
    sensitivity_sum = defaultdict(int)  #i need these total for these as sensitivity for a row   ###have you taken ??? sum of(sum of (sensitivity of all tokens in all tokens of whole_text.append(lemma.name()) + in all tokens of whole_text.append(synset.definition()) + in all tokens of whole_text.extend(synset.examples()) + in all tokens of whole_text.extend([l.name() for l in synset.lemmas()])))??????
    ...
    for token, tag in tagged_tokens:
        for pos_category, tags in pos_map.items():
            if tag in tags:
                sensitivity_sum[pos_category] += ancientness_sensitivity_values.get(pos_category, 0) ######?????????????#i need these total for these as sensitivity for a row   ###have you taken ??? sum of(sum of (sensitivity of all tokens in all tokens of whole_text.append(lemma.name()) + in all tokens of whole_text.append(synset.definition()) + in all tokens of whole_text.extend(synset.examples()) + in all tokens of whole_text.extend([l.name() for l in synset.lemmas()])))??????
    ...
    for synset in wn.all_synsets():
        for lemma in synset.lemmas():
            try:
                ...
                row_frequency = sum(word_freq.get(token.lower(), 0) for token in tokens) ###?????? #i need these total for these as frequency  for a row   ###have you taken ??? sum of(sum of (frequency of all tokens in all tokens of whole_text.append(lemma.name()) + in all tokens of whole_text.append(synset.definition()) + in all tokens of whole_text.extend(synset.examples()) + in all tokens of whole_text.extend([l.name() for l in synset.lemmas()])))??????
                row = {
...
def main():
    try:
        word_frequencies = preprocess_and_calculate_frequencies()###?????? #i need these total for these as frequency  for a row   ###have you taken ??? sum of(sum of (frequency of all tokens in all tokens of whole_text.append(lemma.name()) + in all tokens of whole_text.append(synset.definition()) + in all tokens of whole_text.extend(synset.examples()) + in all tokens of whole_text.extend([l.name() for l in synset.lemmas()])))??????
