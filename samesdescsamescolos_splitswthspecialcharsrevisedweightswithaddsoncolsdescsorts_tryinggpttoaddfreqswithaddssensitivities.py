...
# Define ancientness sensitivity values (as per your example)
...
def collect_wordnet_data(word_freq):
    ...
    # Add color formatting for Excel
    wb = openpyxl.Workbook()
    ws = wb.active
    for idx, row in df.iterrows():
        ws.append(row.tolist())
    
    # Apply color to cells in the 'Frequency * SensitivitySum' column
    max_value = df['Frequency * SensitivitySum'].max()
    min_value = df['Frequency * SensitivitySum'].min()
    color_scale = openpyxl.formatting.rule.ColorScaleRule(
        start_type='min', start_color="FFFFFF", end_type='max', end_color="FF0000"
    )
    ws.conditional_formatting.add(f"D2:D{len(df)+1}", color_scale)
    
    wb.save("samedescssamecols_enhanced_wordnet_data_sorted_with_colors.xlsx")
    return df
...
def main():
    try:
        ...
        print("Data exported successfully with color coding.")
    except Exception as e:
